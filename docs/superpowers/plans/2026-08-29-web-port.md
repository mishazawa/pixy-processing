...
**Interfaces:**
- Produces: `randomExportFilename(extension?: string): string` (unit tested), `downloadCanvasAsPng(canvas: HTMLCanvasElement, filename: string): void` (not unit tested — needs a real DOM/canvas; exercised when the user verifies the app). Not consumed within this plan — Task 16 explicitly does not wire these to a UI control (see Task 16's Interfaces note); available for a future follow-up.
...
### Task 16: Global shortcuts + `App.tsx` wiring
...
**Interfaces:**
- Consumes: `usePixiStore` from `./state/store`; `GridView` from `./components/GridView`; `SingleView` from `./components/SingleView`.
- Produces: the finished `App` component. Terminal task — nothing downstream depends on this. Task 10's `downloadCanvasAsPng`/`randomExportFilename` are NOT wired to a UI button by this task (see the note at the end of this task) — they remain available, tested, unused exports until a follow-up wires an export button.
