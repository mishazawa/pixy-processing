...
class Pop {
  ...
  ArrayList<DNA> lastPool = new ArrayList<>();

  Pop(Pixi app, Sketch sk) {
    this.app = app;
    this.sk = sk;
  }
  ...
  void evolve() {
    ...
    if (pool.size() > 1) {
      for (Artwork a : arts) {
        DNA newDNA = pool.get((int) Rnd.random(pool.size()));
        newDNA = newDNA.sex(newDNA, pool.get((int) Rnd.random(pool.size())));
        a.assignDNA(newDNA);
      }
    }
    lastPool = pool;
